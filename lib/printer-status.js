...
(function (global) {
  ...
  /**
   * Klipper's display_status falls back to the byte position when no `M73`
   * has arrived, and it returns the very same number when it does — so a
   * value that DIFFERS from `virtual_sdcard.progress` is proof that the
   * slicer's own `M73 P` is driving it. Bit-identical means "no M73", and
   * anything else means the printer is being told where it is.
   */
  const M73_SAME_AS_BYTES = 1e-9;

  /**
   * How far through a Moonraker job we are: the printer's own figure first,
   * then layers, then bytes.
   *
   ...
   *
   * ── AND WHY LAYERS ARE NOT THE FIRST CHOICE EITHER ────────────────────────
   *
   * Counting layers assumes every layer costs the same, and a part that is
   * wide at the bottom and slim above breaks that badly. Measured on the
   * shop's own U1, 2026-09-16, printing `Bottom  Body RH_PETG_11h51m.gcode`:
   *
   *     layers            570 of 1067        53.4%
   *     byte position                        76.9%
   *     the printer's own screen             74%
   *
   * Half the layers held three quarters of the work, so the layer count said
   * the job was barely half done. Khayt extrapolated SEVEN HOURS AND TWENTY
   * MINUTES left; the machine's own display said under three. The shop was
   * looking at both numbers at once.
   *
   * The signal the printer itself shows is `M73 P` — the percentage the
   * SLICER computed from its own time estimate, written into the G-code and
   * relayed by Klipper as `display_status.progress`. It is the only one of
   * the three that is about TIME rather than about geometry or file layout,
   * which is exactly what an estimate of time left needs. Khayt had never
   * asked for it: `display_status` was not in the Moonraker query at all.
   *
   * So: the printer's own figure when it has one, layers when it does not,
   * bytes as the floor. The two earlier measurements still hold — the relief
   * whose bytes read 0.7% at 19% done had no M73 either way, and falls to
   * layers exactly as before.
   *
   * @returns {{percent: number, source: 'm73'|'layers'|'bytes'}}
   */
  function moonrakerProgress(printStats, virtualSdcard, displayStatus) {
    const bytes = Number(virtualSdcard && virtualSdcard.progress);
    const shown = Number(displayStatus && displayStatus.progress);
    // `M73` is live when Klipper is not simply echoing the byte position.
    if (Number.isFinite(shown)
        && (!Number.isFinite(bytes) || Math.abs(shown - bytes) > M73_SAME_AS_BYTES)) {
      return { percent: normalizeProgress(shown * 100), source: 'm73' };
    }
    const byLayer = layerProgressPct(printStats && printStats.info);
    if (byLayer !== null) return { percent: byLayer, source: 'layers' };
    return { percent: normalizeProgress(Number.isFinite(bytes) ? bytes * 100 : 0), source: 'bytes' };
  }
