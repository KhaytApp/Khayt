...
(function (global) {
  ...
  function apply(settings, form, ctx) {
    ...
    // Minutes one spool change takes, for the colour-grouped queue
    // (lib/swap-queue.js). An ESTIMATE the shop owns; 0-60, like the rule's.
    if (has(f, 'swapMinutes'))          out.swapMinutes          = clamp(0, 60, num(f.swapMinutes, 3));
    ...
    // ── NTFY, WHEN A FORM CARRIES IT ──────────────────────────────────────
    //
    // A push channel for printer alerts (`lib/alert-routes.js`). Merged over
    // what is stored; the access token is OPAQUE — sealed by the host — and
    // follows Telegram's rule: absent keeps it, an empty string clears it.
    if (has(f, 'ntfy') && f.ntfy && typeof f.ntfy === 'object') {
      const g = f.ntfy;
      const held = s.ntfy || {};
      const ev = { ...(held.events || {}) };
      for (const k of ['error', 'offline', 'stall', 'runout']) {
        if (g.events && has(g.events, k)) ev[k] = !!g.events[k];
      }
      out.ntfy = {
        ...held,
        enabled: has(g, 'enabled') ? !!g.enabled : !!held.enabled,
        server: has(g, 'server') ? String(g.server == null ? '' : g.server).trim() : (held.server || ''),
        topic: has(g, 'topic') ? String(g.topic == null ? '' : g.topic).trim() : (held.topic || ''),
        token: has(g, 'token') ? String(g.token || '') : (held.token || ''),
        events: ev,
      };
    }
