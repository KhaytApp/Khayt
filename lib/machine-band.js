...
function downtimeIn(machine, from, to) {
  ...
  for (const b of blocks) {
    ...
    // `reason` is the field Khayt's machine modal writes; `note` is accepted
    // because nothing stops a caller using the more obvious name. Reading only
    // `note` — which this did — meant every window a shop actually recorded
    // drew with a blank title.
    out.push({ startsAt, endsAt, note: String((b && (b.reason || b.note)) || '') });
  }
...
function band(input) {
  ...
  const rows = machines.map(m => {
    ...
    // ── NOT ANSWERING ────────────────────────────────────────────────────
    //
    // What the header promises for "one whose poll is failing": no blocks,
    // `known: false`, and out of the capacity totals. It was only ever done for
    // a machine the book said was printing; an idle printer that stopped
    // answering came out `free` with forty-eight hours, counted into the total,
    // beside a dashboard saying "not answering". `error` is the poller's (see
    // printer-poll-cache.mergePollFailure), so it means exactly that.
    if (reading && reading.error && !running) {
      return {
        machineId: id, name: String(m.name || ''), state: 'offline',
