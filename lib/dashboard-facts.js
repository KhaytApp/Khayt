...
function dashboardFacts(input) {
  ...
  // Finished jobs charged nothing that still count as trade. Beside `attn`
  // rather than inside it: see `selectUnpricedFinished`.
  const unpricedSelect = inp.attention && typeof inp.attention.selectUnpricedFinished === 'function'
    ? inp.attention.selectUnpricedFinished : null;
  let unpriced = { count: 0, ids: [] };
  if (unpricedSelect) {
    try { unpriced = unpricedSelect(orders); } catch (e) { /* advisory, like attn */ }
  }
