...
(function (global) {
  ...
  function computePnl(input) {
    ...
    const netProfit = grossProfit - expensesTotal - waste;
    return {
      label: input.label || '',
   ...
   *
   * `ctx`: `{ settings, clients, currencies, now, granularity }`. The siblings
  ...
  function pnlByPeriod(orders, expenses, ctx) {
    ...
    return Object.keys(byQuarter).sort().reverse().map((key) => {
      ...
      return {
        ...
        net: round2(row.revenue - row.cogs - row.expenses - row.waste - fixed),
