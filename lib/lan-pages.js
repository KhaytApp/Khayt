...
(function (global) {
  ...
  /**
   * The shop's own name, for a page a customer or a phone will see.
   *
   * `settings.shopName` FIRST, then the business name the rest of the app
   * uses — and that second half is the whole point. Nothing anywhere writes
   * `shopName`: the name a shop actually types lives in `bizEn`/`bizAr`, read
   * bilingually through `content-languages`. So the queue page and the phone's
   * home-screen icon both said "Khayt" for every shop that ever installed
   * them, while the quote page two files away said the shop's real name — it
   * had this fallback and these did not.
   *
   * Same order as `KhaytEngine.lanQuoteShopName`, deliberately, so one shop
   * cannot be two names across its own pages.
   */
  function shopNameOf(settings) {
    const st = settings || {};
    if (st.shopName) return String(st.shopName);
    const langs = (typeof globalThis !== 'undefined') && globalThis.KhaytContentLanguages;
    if (langs && typeof langs.read === 'function') {
      const biz = langs.read(st, 'biz', st.lang || 'en', st);
  ...
  function manifest(store) {
    const s = store || {};
    const shopName = shopNameOf(s.settings);
    return {
      name: shopName + ' Queue',
      short_name: shopName,
      start_url: '/',
...
self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(ks=>Promise.all(ks.filter(k=>k!==CACHE).map(k=>caches.delete(k))))));`;
  ...
  function queuePage(store, opts) {
    ...
    const shopName = esc(shopNameOf(s.settings));
...
setTimeout(()=>location.reload(),30000);
  ...
  const api = { statusJson, queueJson, manifest, serviceWorker, queuePage, notFound,
    shopNameOf, NODE_ENDPOINTS, SECURITY_HEADERS };
