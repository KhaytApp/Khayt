...
// The gates in front of this server. Shared so a second implementation of a
// brute-force lockout cannot exist — see the block below `safeTokenEqual`.
const LanAuth = require('./lan-auth.js');
// What the phone is shown — the queue page, the status and queue JSON, the
// PWA files. Lifted so the Mac app serves the same bytes; see the module.
const LanPages = require('./lan-pages.js');
const LanIntake = require('./lan-intake.js');
const LanQuotePage = require('./lan-quote-page.js');
const LanOrderPage = require('./lan-order-page.js');
const LanCalendar = require('./lan-calendar.js');
...
const storefrontWebhook = require('./storefront-webhook.js');
const carrierWebhook = require('./carrier-webhook.js');
...
// Salla/Zid and the carriers sign the BODY only, so a captured delivery is
// valid for ever. The rule and its reasons are lib/webhook-replay.js: thirty
// days, 10,000 entries, SHA-256 of the signature, kept in a file beside the
// book so a restart does not reset it (SEC-010). In-memory until
// registerLanServer is handed the file's path — which is what tests get.
const { createReplayGuard } = require('./webhook-replay');
let _replayGuard = createReplayGuard();

function isReplayedWebhook(signature, now = Date.now()) {
  return _replayGuard.isReplay(signature, now);
}
...
// behaviour untouched).
/* ── THE GATES IN FRONT OF THIS SERVER LIVE IN `lib/lan-auth.js` ───────────────
 *
 * The lockout arithmetic, both global gates, the PIN-strength rule, the
 * client-IP choice behind a tunnel and the HTML escape are shared now: a LAN
 * server is a LAN server wherever it runs, and two implementations of a
 * brute-force lockout is two chances to write the one that does not lock.
 *
 * `safeTokenEqual` above deliberately did NOT move. A constant-time compare is
 * a primitive, not a rule, and `crypto.timingSafeEqual` is the right one here.
 *
 * Re-exported under their old names because every call site in this file, and
 * `module.exports` at the bottom, has always reached them this way. */
const {
  bumpFailure, isLockedOut, sweepFailedAttempts,
  globalWindowGate, globalAuthThrottle,
  weakTunnelPinWarning, tunnelClientIp,
...
/* An online order for something already printed comes off the shelf, and a
 * delivery the book already holds is not recorded twice: both are
 * `lib/storefront-webhook.js` now, which the Mac's LAN server runs too. */
...
const LAN_SPOOL_FIELDS = new Set([
  ...
  // The product barcode (UPC/EAN) off the box. The companion scans it and looks
  // the filament up — first on this shop's own shelf, by this field — so a
  // roll booked in without it is a roll the next identical box cannot find.
  'barcode',
]);
...
function pickLanSpoolFields(raw) {
  ...
  // A GTIN is 8 to 14 digits and nothing else. Anything else stored here would
  // be matched against every future scan, so it is dropped rather than kept.
  if (spool.barcode !== undefined && !/^\d{8,14}$/.test(String(spool.barcode))) delete spool.barcode;
  return spool;
}
...
/** @param {unknown} v */
// `sanitizeLanHttpUrl` is `lib/lan-auth.js`'s too — see the block below
// `safeTokenEqual`.
const sanitizeLanHttpUrl = LanAuth.sanitizeLanHttpUrl;

/* ── A CSP CONSTRAINS ONLY WHAT IT NAMES ───────────────────────────────────────
 *
 * This policy listed four directives, and every directive it did not list was
 * therefore unrestricted. Not loosely restricted — unrestricted. The pages this
 * covers are the ones a shop hands to its customers, and over the remote tunnel
 * they are reachable from the internet, so the gap was worth closing:
 *
 *   form-action   absent  →  an injected <form> could post the customer's name,
 *                            email and phone off the intake page to any host.
 *   connect-src   absent  →  fetch() to anywhere.
 *   img-src       absent  →  a pixel is enough to exfiltrate a quote's contents.
 *
 * `script-src` already carries `'unsafe-inline'` and has to: these pages embed
 * their script (the tracking page's 30-second reload, the intake form's submit).
 * That is the reason the rest matters rather than a reason to shrug — inline
 * script cannot be the line that holds, so the line has to be drawn around what
 * an injection could REACH.
 *
 * `default-src` catches the ones that fall back to it. `form-action` and
 * `frame-ancestors` do NOT fall back, in any CSP level, so they are named
 * explicitly — which is the whole lesson of this function repeating itself.
 *
 * Verified before tightening rather than after: these pages load no image, no
 * font, no stylesheet and no script that is not their own, and their three
 * fetches (`/api/intake`, `/api/intake/estimate`, `/api/survey`) are same-origin.
 * `'self'` costs them nothing.
 *
 * The same shape as everything else found in this file — a guard that is absent
...
function registerLanServer(deps) {
  ...
  // Replays are remembered across restarts once there is somewhere to keep
  // them. Written to a temp file and renamed, so a crash mid-write leaves the
  // previous list rather than half of one.
  if (typeof webhookSeenPath === 'function') {
    _replayGuard = createReplayGuard({
      load: () => { const p = webhookSeenPath(); return fs.existsSync(p) ? fs.readFileSync(p, 'utf8') : null; },
      save: (text) => { const p = webhookSeenPath(); fs.writeFileSync(p + '.tmp', text, 'utf8'); fs.renameSync(p + '.tmp', p); },
    });
  }
...
ipcMain.handle('hub:start-lan-server', async (_e, { port = 3219, pin = '', bindLan = 'loopback' } = {}) => {
  ...
  // remote memory-exhaustion DoS through the tunnel. The webhook replay guard sweeps; this
  ...
  const quoteExpiredHtml = () => LanQuotePage.notice('expired');
  return new Promise(resolve => {
    try {
      lanServer = http.createServer((req, res) => {
        try {   // nothing below may throw without answering — see the catch.
        ...
        //
        // Everything in this list carries its OWN check — a per-order tracking
        // token, a calendar token, a webhook secret — and must therefore be
        // exempt from the owner-PIN gate below. Not because it is unprotected,
        // but because the gate answers 401 BEFORE the route's own check ever
        // runs, and the customer holding a tracking link has no PIN to send.
        //
        // `/status/` and `/calendar.ics` were missing from it. Both verify a
        // token; neither could be reached once the shop set a PIN — which the
        // app pushes every shop to do, because writes require one. The tell was
        // that `/order/:id` (in this list) served the same tracking token
        // happily while `/status/:id` (not in it) refused the same link, and
        // that the Mac app served both. A route added here must ALWAYS carry
        // its own auth; `test/lan-self-authenticating-routes.test.js` speaks
        // HTTP to each of them and is the guard against the next omission.
        ...
        // ── EXEMPT FROM THE PIN, BUT NOT THEREFORE PUBLIC ─────────────────
        //
        // These carry their own check — `/status/:id` a per-order tracking
        // token, `/calendar.ics` a calendar token — so the owner-PIN gate must
        // not answer 401 before that check is ever reached. The customer
        // holding a tracking link has no PIN to send, and neither has a
        // calendar app; both routes returned 401 to a perfectly good token for
        // any shop that had set one, which is every shop that writes through
        // the API, because writes require one.
        //
        // SEPARATE FROM `isAlwaysPublic` ON PURPOSE. That list also grants
        // `Access-Control-Allow-Origin: *`, and these two must not have it:
        // `/calendar.ics` accepts the owner PIN as an alternative to its token,
        // so a wildcard would let any web page in the world guess the shop's
        // PIN through the shop's own browser. Exempting a route from the PIN
        // and opening it to every origin are different questions, and the one
        // list was answering both.
        const isSelfAuthenticated =
          (pathname.startsWith('/status/') && req.method === 'GET') ||
          (pathname === '/calendar.ics' && req.method === 'GET');
        ...
        if (!apiTokenScoped && !isAlwaysPublic && !isSelfAuthenticated && !isIntakePublicGet && !isIntakeSessionPost && !isIntakeSubmitPost && !isIntakeEstimatePost && (requirePin || (pin && !isSurveyEndpoint))) {
        ...
        // The form's styles and template live in `lib/lan-intake.js` — the Mac app serves the same page.
        ...
        const renderIntakeFormPage = LanIntake.formPage;
        ...
        if (pathname === '/api/status') {
          ...
          res.end(JSON.stringify(LanPages.statusJson(store, localDay())));
          ...
          if (!checkPinForGet()) return;
          res.writeHead(200);
          res.end(JSON.stringify(LanPages.queueJson(store)));
        ...
        // either. Both are defaults rather than something remembered here.
          ...
          req.on('end', async () => {
            try {
              ...
              for (const key of ['brand', 'vendor', 'notes', 'filament', 'sku', 'barcode']) {
        ...
        // ── Customer survey submission (public — protected by one-time token) ──
          ...
          const shopName = store.settings?.shopName
            ...
            || 'Khayt';
          const invalidLinkHtml = LanQuotePage.notice('invalid_link');
          setLanHtmlSecurityHeaders(res);
          res.setHeader('Content-Type', 'text/html; charset=utf-8');
          res.setHeader('Cache-Control', 'no-cache');
          if (!order) {
            res.writeHead(404);
            res.end(LanQuotePage.notice('quote_not_found'));
          ...
          req.on('end', async () => {
            try {
              ...
              if (idx === -1) {
                ...
                res.end(LanQuotePage.notice('order_not_found'));
                return;
              }
              ...
              if (!verifyQuoteApprovalToken(storeData.printLog[idx], approvalTok)) {
                ...
                res.end(LanQuotePage.notice('invalid_link_approve'));
                return;
              }
              ...
              if (result.error === 'cannot_approve') {
                ...
                res.end(LanQuotePage.notice('cannot_approve'));
                return;
              }
              await updateStoreOnDisk((cur) => {
                const next = { ...cur, printLog: [...(cur.printLog || [])] };
          ...
          res.setHeader('Content-Type', 'text/html; charset=utf-8');
              res.writeHead(200);
              res.end(LanQuotePage.notice('approved', { project: projectName }));
            } catch (e) {
              res.writeHead(400, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ error: String(e) }));
            }
          ...
          if (!order) {
            res.writeHead(404);
            res.end(LanOrderPage.notice('order_not_found'));
            ...
            if (!order.trackingToken || !verifyOrderAccessToken(order, accessTok, 'trackingToken')) {
              res.writeHead(403);
              res.end(LanOrderPage.notice('invalid_tracking_link'));
              return;
            }
            res.end(LanOrderPage.trackingPage(order, store));
          }

        } else if (pathname === '/manifest.json' && req.method === 'GET') {
          res.setHeader('Content-Type', 'application/manifest+json');
          res.writeHead(200);
          res.end(JSON.stringify(LanPages.manifest(store)));
          ...
          res.writeHead(200);
          res.end(LanPages.serviceWorker());
          ...
          const html = LanPages.queuePage(store, { now });
          setLanHtmlSecurityHeaders(res);
          res.setHeader('Content-Type', 'text/html; charset=utf-8');
          res.setHeader('Cache-Control', 'no-cache');
          res.writeHead(200);
        ...
        // ── iCal feed ───────────────────────────────────────────
          ...
          // Recording a failure below is only half of a lockout; without this
          // the route keeps answering and stays a guessing oracle for the PIN
          // however many times it has already been wrong.
          if (isLockedOut(failedAttempts.get(ip), Date.now())) {
            res.writeHead(429, { 'Content-Type': 'text/plain; charset=utf-8' });
            res.end('Too many attempts — try again in 1 minute.');
            return;
          }
          const authorized = (calToken && tokenParam && safeTokenEqual(tokenParam, calToken))
            || (pin && pinParam && safeTokenEqual(pinParam, pin));
          if (!authorized) {
            // ── AND IT COUNTS AGAINST THE LOCKOUT ───────────────────────
            //
            // This route is exempt from the PIN gate (above) so a calendar app
            // holding only a token can reach it. That exemption also takes it
            // out of the gate's brute-force counter — and this route ACCEPTS
            // THE PIN, which is four digits in the field's own placeholder. A
            // public, PIN-accepting, unthrottled endpoint is a guessing oracle
            // for the credential that opens every other route, so the failure
            // is recorded here instead. Only a PIN attempt counts: a calendar
            // app that polls with a stale token should not lock the owner out.
            if (pinParam) {
              const now = Date.now();
              const ipData = failedAttempts.get(ip) || { count: 0, resetAt: 0 };
              failedAttempts.set(ip, bumpFailure(ipData, now, { lockoutMs: LOCKOUT_MS }));
          ...
          const icalBody = LanCalendar.feed(store);
        ...
        // file, and what it returns is explicitly not binding.
          ...
          req.on('end', async () => {
            try {
              ...
              const parsed = parseLanJsonBody(body, {});
              // The rule is `lib/lan-intake.js` now — what a posted form becomes, or
              // why it is refused — so the Mac app records the same entry. The
              // estimate is recalled here BY REFERENCE and handed in: a browser can
              // post any number it likes; the shop must see what the server said.
              const outcome = LanIntake.submission(parsed, {
                shopName: STORE().settings?.shopName || 'this shop',
                id: uniqueLanId('intake'),
                quoted: recallEstimate(parsed.estimateRef),
              });
              if (!outcome.ok) {
        ...
        // ── Salla inbound order webhook ─────────────────────────
          ...
          req.on('end', async () => {
            try {
              ...
              if (storefrontWebhook.alreadyRecorded('salla', parsed, STORE().printLog)) {
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ ok: true, duplicate: true }));
                return;
              }
              // The order it becomes, the duplicate check redone INSIDE the
              // write (a retry arriving while the first write is in flight
              // passes the check above), and what it takes off the shelf are
              // all `lib/storefront-webhook.js` — the rule the Mac runs too.
              const ctx = { id: uniqueLanId('salla'), day: localDay(), at: new Date().toISOString() };
              let recorded = null;
              await updateStoreOnDisk((cur) => {
                const result = storefrontWebhook.record('salla', parsed, cur, ctx);
                recorded = result.order;
                return result.store;
              });
              // Lost the race to its own retry: the book already held it, so
              // nothing was written and there is nothing to show the window.
              if (!recorded) {
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ ok: true, duplicate: true }));
                return;
              ...
              if (getMainWindow() && !getMainWindow().isDestroyed()) {
                getMainWindow().webContents.send('lan-order-updated', recorded);
              }
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ ok: true }));
            } catch (e) {
        ...
        // ── Zid inbound order webhook ───────────────────────────
          ...
          req.on('end', async () => {
            try {
              ...
              // the ten-minute TTL that the signature cache above does not. A
              // provider retry is an ordinary event, not an attack.
              if (storefrontWebhook.alreadyRecorded('zid', parsed, STORE().printLog)) {
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ ok: true, duplicate: true }));
                return;
              }
              ...
              // passes the check above), and what it takes off the shelf are
              // all `lib/storefront-webhook.js` — the rule the Mac runs too.
              const ctx = { id: uniqueLanId('zid'), day: localDay(), at: new Date().toISOString() };
              let recorded = null;
              await updateStoreOnDisk((cur) => {
                const result = storefrontWebhook.record('zid', parsed, cur, ctx);
                recorded = result.order;
                return result.store;
              });
              // Lost the race to its own retry: the book already held it, so
        ...
        // ── Carrier shipping-status webhooks (SMSA / Aramex / SPL) ──
          ...
          req.on('end', async () => {
            try {
              ...
              // Reading the payload, finding the job and moving it forward are
              // `lib/carrier-webhook.js` — the rule the Mac's server runs too.
              const parsed = safeJsonParse(bodyBuf.toString('utf8'));
              const evt = carrierWebhook.read(carrierId, parsed, req.headers, STORE().settings?.shipping?.[carrierId] || {});
              ...
              // An unknown tracking number keeps its 200 — a different answer
              // would confirm which parcels this shop holds — and an event that
              // arrived out of order changes nothing. Neither is worth a write.
              const at = evt.at || new Date().toISOString();
              if (carrierWebhook.apply(STORE(), evt, at).outcome === 'advanced') {
                // Applied again INSIDE the write: two events arriving together
                // would otherwise each build the trail from the same base.
                let written = null;
                await updateStoreOnDisk((cur) => {
                  const result = carrierWebhook.apply(cur, evt, at);
                  written = result.order;
                  return result.store;
                });
                // The record that was written, not a draft built before the
                // write — the window used to be told the latter.
                if (written && getMainWindow() && !getMainWindow().isDestroyed()) {
                  getMainWindow().webContents.send('lan-order-updated', written);
                }
              }
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ ok: true }));
          ...
          res.writeHead(404);
          res.end(JSON.stringify(LanPages.notFound(LanPages.NODE_ENDPOINTS)));
        }
        } catch (err) {
          // ── NOTHING ABOVE MAY THROW WITHOUT AN ANSWER ─────────────────
          //
          // Node's request listener has no error handling of its own: a throw
          // inside it escapes to `uncaughtException` and the SOCKET IS NEVER
          // ANSWERED. The client waits for its own timeout — and this app's
          // process-level handler files the throw as a crash report, so one
          // malformed request produced a hung connection and a bogus crash
          // together.
          //
          // `GET /status/%` did exactly that. That route decodes the order id
          // with `decodeURIComponent`, a lone percent is a `URIError`, and the
          // route is public: no PIN, no token, anyone on the shop's Wi-Fi.
          // Seven other routes decode a path segment the same way, and the
          // cookie parser decodes a header the customer's browser sends.
          //
          // Each could have been fixed on its own. This is the fix that also
          // covers the next route somebody adds.
          //
          // A malformed request is not a server fault: say so and stay up. The
          // reason is deliberately not echoed back — it would hand a prober a
          // description of the internals for free.
          try {
            if (!res.headersSent) {
              res.writeHead(400, { 'Content-Type': 'application/json' });
          ...
          console.error('lan-server: request failed —', err && err.message ? err.message : err);
        }
      });
