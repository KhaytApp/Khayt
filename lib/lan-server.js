        ...
        // ── Carrier shipping-status webhooks (SMSA / Aramex / SPL) ──
          ...
          req.on('end', async () => {
            try {
              ...
              try { carriersLib = require('./carriers.js'); } catch (_) { carriersLib = null; }
              // What a carrier event writes onto the job — `shippedAt`,
              // `deliveredAt`, or both. One rule, so the webhook and the
              // shipping dialog cannot disagree about when a parcel left.
              let statusLib = null;
              try { statusLib = require('./order-status.js'); } catch (_) { statusLib = null; }
              const statusRules = () => statusLib || { stampFromShipping: () => false };
              ...
              if (advanced !== order.shippingStatus) {
                ...
                statusRules().stampFromShipping(order, advanced, at);
                ...
                await updateStoreOnDisk((cur) => {
                  ...
                  statusRules().stampFromShipping(next, step, at);
                  log[i] = next;
                  return { ...cur, printLog: log };
                });
                if (getMainWindow() && !getMainWindow().isDestroyed()) {
