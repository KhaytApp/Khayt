...
const shelfSale = require('./shelf-sale.js');
 ...
 * publishes and sells against, and until now one screen in the whole app wrote
 * it: a person typing a number. Nothing took one off. So a shop that printed
 * twelve, listed twelve and sold four went on publishing twelve — and the next
 * publish put the four that were gone back on sale.
 *
 * Called INSIDE `updateStoreOnDisk`, and it has to be: reading the shelf,
 * subtracting from it and writing it back is a read-modify-write, and two
 * deliveries arriving together outside the chain would each read the same
 * figure and each write their own.
 *
 ...
 */
function takeOnlineOrderOffTheShelf(source, parsed, cur, order) {
        ...
        // ── Salla inbound order webhook ─────────────────────────
          ...
          req.on('end', async () => {
            try {
              ...
              let recorded = newOrder;
              await updateStoreOnDisk((cur) => {
                const log = [...(cur.printLog || [])];
                if (storefrontOrders.alreadyRecorded(log, 'salla', sallaRef)) return cur;
                // The shelf is read and written in here for the same reason the
                // duplicate check is: it is a read-modify-write, and two
                // deliveries arriving together would each read the same count.
                const taken = takeOnlineOrderOffTheShelf('salla', parsed, cur, newOrder);
                recorded = taken.order;
                log.unshift(recorded);
                return { ...taken.store, printLog: log };
              });
              if (getMainWindow() && !getMainWindow().isDestroyed()) {
          ...
          req.on('end', async () => {
            try {
              ...
              await updateStoreOnDisk((cur) => {
                ...
                // See the Salla handler: inside the chain, because the shelf is
                // read, subtracted from and written back.
                const taken = takeOnlineOrderOffTheShelf('zid', parsed, cur, newOrder);
                recorded = taken.order;
                log.unshift(recorded);
                return { ...taken.store, printLog: log };
              });
