...
// What may go up is cloud-outbox's decision, not this module's: secrets masked,
// the device-private fields (ntfy topic, webhook URLs) masked, and anything
// sealed on disk masked. The renderer's snapshot already carries masks for the
// first; the second it holds in the clear, because the settings screen shows
// them. Loaded for its side effect: forCloud reads the lists from the global.
require('./store-secret-paths.js');
const { forCloud } = require('./cloud-outbox.js');
...
function httpFailure(what, res) {
  ...
  const err = new Error(said ? `${what}: ${said} (${code})` : `${what}: ${code}`);
  // Kept as a number so a caller can tell a refusal that retrying will never
  // fix (401, 403, 412, 413) from one that time might (5xx).
...
function createCloudBackend({ transport, crypto, shopId, getDek, sync, deltaWrites, viewCache, getLocalSnapshot }) {
  ...
  async function pull() {
    ...
    try {
      ...
      // No store on the server — so its head is 0, whatever this device last
      // pushed. Keeping the old rev was a trap: every push then guarded on a
      // head that no longer exists, earned 409 `{ rev: 0 }`, pulled this same
      // 204 and tried again, and the shop never synced. A 204 is safe to
      // believe since a store row whose object is missing answers 500 instead
      // (khayt-cloud #68): this is "nothing here", never "something was lost".
      if (res.status === 204) {
        forgetServerView();
        statusVal = 'idle';
        return { store: null, rev: 0 };
      }
  ...
  async function push(snapshot) {
    ...
    try {
      ...
      const ciphertext = crypto.encryptStore(forCloud(snapshot), getDek());
  ...
  async function pushDelta(snapshot) {
    ...
    // `rev: 0` is the server saying it holds no base at all, so there is no
    // chain to append to and nothing a whole store could overwrite. Send the
    // whole store at baseRev 0, as the Mac app does; the guard still refuses it
    // if another device got there first.
    if (res.status === 409 && res.body && res.body.rev === 0) {
      forgetServerView();
      return null;
    }
    if (res.status === 409) {
