...
(function (global) {
  ...
  function looksLikeItsName(ext, header, size) {
    ...
    switch (ext) {
      ...
      // Both are zip containers, so both begin PK\x03\x04. A 3MF IS a zip
      // with a manifest; a plain zip is a shop's own folder of models, which
      // the library expands on import.
      case '3mf':
      case 'zip': return hex.startsWith('504b0304');
      ...
      // ── ARCHIVES A MODEL PACK ACTUALLY ARRIVES IN ────────────────────
      //
      // Every one of these has a magic number in its opening bytes, so the
      // test is the same kind as the zip's above and not a weaker one.
      //
      // A bare `.tar` is deliberately absent: its magic lives at offset 257,
      // which is not in the header this is given, and a check that always
      // passed would be worse than no support at all.
      case 'rar': return hex.startsWith('526172211a07');
      case '7z':  return hex.startsWith('377abcaf271c');
      case 'tgz':
      case 'gz':  return hex.startsWith('1f8b');
  ...
  function verdict(facts, opts) {
    ...
    //
    // A plain `zip` answers for exactly the same things a 3MF does, and for the
    // same reason: both are zip containers, and a container that expands to a
    // thousand times what arrived, or that names a member `../../etc/passwd`,
    // is the same problem whichever extension it wears. The library expands a
    // shop's own zip on import, which is a smaller blast radius than a
    // stranger's upload but not a different rule.
  ...
  /** Extensions that are a zip container and answer for what is inside them. */
  /**
   * Archives whose MEMBERS can be listed before anything is unpacked.
   *
   * `rar`, `7z` and `tgz` are deliberately NOT here. Nothing in either app can
   * read their directories without a library neither ships, so there are no
   * entries to check — and the checks below refuse an archive that declares
   * none. The host that opens one caps what it writes as it goes instead,
   * which is a stronger promise than trusting a declared size: a bomb lies in
   * its own headers.
   */
  const ARCHIVES = ['3mf', 'zip'];

  function refuse(reason) { return { ok: false, reason }; }

  const api = { verdict, unsafeName, looksLikeItsName, ARCHIVES,
