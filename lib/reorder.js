...
/**
 * WHAT A GRAM OF THIS MATERIAL COSTS, for a purchase order to be priced at.
 *
 * ── THE FAULT THIS CARRIES THE SCAR OF ────────────────────────────────────
 *
 * A purchase order's `qty` is in GRAMS and its `unitPrice` is per gram, so the
 * two multiply out directly. `item.cost` is the cost of the WHOLE SPOOL — the
 * stock valuation divides it by `spoolWeight`, the CSV import maps `costPerKg`
 * onto it, and `calculator-cost.js` divides it too. Returning it undivided made
 * every auto-drafted order about 1000x too expensive: 750 g of an 85 SAR spool
 * asking for 63,750 instead of 63.75.
 *
 * The supplier branch always divided. Only the fallback forgot, which is why
 * `lib/po-audit.js` exists at all.
 *
 * Lifted out of `renderer/inventory.js` so the macOS app can draft an order
 * without a second copy of the division that went wrong.
 *
 * @param {object} item        the spool
 * @param {object[]} suppliers the shop's suppliers, for a quoted price
 * @returns {{perG: number, supplierId: string|null, supplierName: string}}
 *   `perG` is 0 when nothing prices this material — the caller writes no price
 *   rather than a price of nothing.
 */
function perGramPriceFor(item, suppliers) {
  const row = item || {};
  const quoted = supplierPriceFor(suppliers, row.material);
  if (quoted && quoted.pricePerKg > 0) {
    return {
      perG: Math.round((quoted.pricePerKg / 1000) * 1000) / 1000,
      supplierId: quoted.supplierId,
      supplierName: quoted.supplierName,
    };
  }
  const perSpool = +row.cost || 0;
  const perG = perSpool > 0
    ? perSpool / Math.max(1, +row.spoolWeight || 1000)
    : (row.costPerKg ? +row.costPerKg / 1000 : 0);
  return {
    perG: perG > 0 ? Math.round(perG * 1000) / 1000 : 0,
    supplierId: row.supplierId || null,
    supplierName: '',
  };
}

const api = { DAY_MS, perGramPriceFor, completionMs, runway, runwayByItem, consumptionByItem, committedByItem, reorderSuggestions, reorderText, itemsNeedingDraftPo, supplierPriceFor, famToken };
