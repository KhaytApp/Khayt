...
function isBlockedHost(rawHost) {
  ...
  if (v4) {
    ...
    //
    // THIS IS A PRIVATE NETWORK THAT IS NOT RFC 1918. A shop that runs
    // Tailscale to reach its printers from home has its machines, its NAS and
    // whatever else is on that tailnet addressed in 100.64–100.127, and this
    // list blocked 10/8, 172.16/12 and 192.168/16 while letting all of it
    // through. `100.100.100.100` is Tailscale's own MagicDNS resolver.
    //
    // The range is /10, so the second octet runs 64–127 — not the whole of
    // 100/8, which is ordinary public space and must stay reachable.
