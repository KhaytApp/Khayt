...
(function (global) {
  ...
  function publishable(products, settings, lang) {
    ...
    return (Array.isArray(products) ? products : []).slice(0, MAX_PRODUCTS).filter((p) =>
      p && String(p.nameEn || CL.read(p, 'name', lang || 'en', settings || null) || '').trim());
  }
  ...
  const api = { build, publishable, parseOptionGroups, MAX_PRODUCTS };
