...
(function (global) {
   ...
   */
  function publishable(products, settings, lang, opts) {
     ...
     * and settings do not.
     *
     * `{ includeHidden: true }` is for a screen that LISTS products to edit
     * their storefront entries (the desktop's Storefront dialog): a product
     * hidden on one device must stay visible there, or nobody could un-hide it.
     * Hidden ones then count toward the cap, as they did before hiding existed.
     * build() never passes it. */
    const withHidden = !!(opts && opts.includeHidden);
    return (Array.isArray(products) ? products : [])
      .filter((p) => p && (withHidden || !p.storefrontHidden))
