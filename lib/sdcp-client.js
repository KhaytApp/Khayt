 ...
 */
(function (global) {
  // REQUIRED OUTRIGHT. `sdcp.js` DOES publish `globalThis.KhaytSdcp` now — the
  // Mac app loads it in JavaScriptCore — but this file still cannot go
  // anywhere: it speaks UDP and WebSocket and requires at module scope. The
  // portable half is `lib/sdcp.js` underneath and `lib/sdcp-reply.js` beside
  // it, and both apps use those.
  const sdcp = require('./sdcp.js');
  const reply = require('./sdcp-reply.js');
  ...
  // Which frame is the answer lives in `lib/sdcp-reply.js`, so the Mac app
  // decides it the same way. Re-exported because callers and tests here have
  // always reached it through this module.
  const forThisBoard = reply.forThisBoard;
  ...
  function fetchStatus(opts) {
    ...
    return askOnce({
      ...
      pick: (msg) => {
        const taken = reply.takeStatus(msg, mainboardId);
  ...
  function fetchAttributes(opts) {
    ...
    return askOnce({
      ...
      pick: (msg) => {
        const taken = reply.takeAttributes(msg, mainboardId);
        if (!taken) return null;
        if (taken.error) throw new Error(taken.error);
        return taken.attributes;
      },
    });
  }
