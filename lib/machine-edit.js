...
(function (global) {
  ...
  function applyEdit(machine, input, ctx) {
    ...
    /* ── WHAT IS LOADED, WHEN THE PRINTER CANNOT SAY ─────────────────────
     *
     * `loaded` is `[{ slot, hex, material }]`, one per head holding a spool,
     * typed by the shop for a printer that does not report it (a U1 does, and
     * its own reading wins). The library's "Ready on" chip reads it through
     * lib/loaded-colours.js, which is also what cleans it here: a colour that
     * is not one is dropped rather than guessed, and the list is capped at
     * sixteen (not at `maxColors`, which is often unknown and would silently
     * drop heads the shop typed). An empty list means "nothing loaded". */
    if (has('loaded')) {
      const LC = (typeof require === 'function')
        ? (() => { try { return require('./loaded-colours.js'); } catch (e) { return null; } })()
        : (typeof globalThis !== 'undefined' ? globalThis.KhaytLoadedColours : null);
      const rows = Array.isArray(i.loaded) ? i.loaded : [];
      const clean = LC ? LC.normalizeLoaded(rows) : [];
      m.loaded = clean.slice(0, 16).map((x) => ({
        slot: x.slot, hex: x.hex, material: x.material.slice(0, 40),
      }));
    }
    ...
    if (has('printerApi')) {
      ...
      const next = {
        // EVERYTHING STORED IS CARRIED, then the fields this edit owns are
        // written over it. The record was rebuilt from a fixed list, which
        // dropped `serial` — the one thing a Bambu or an Elegoo is addressed
        // by — so correcting a Bambu's host on the Mac silently cut it off.
        ...was,
        type: trim(a.type),
        host: trim(a.host),
        // A TCP port is 1-65535. 88830 was stored as typed and crashed the Mac
        // app's printer poll on every launch.
