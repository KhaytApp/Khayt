...
- **Stable:** `3.10.x` — latest tag **v3.10.1**, published 2026-09-26 ([releases/latest](https://github.com/khaytapp/Khayt/releases/latest)), a patch cut and tagged the same day, all three platforms: net profit that counted filament twice now counts it once, when it is used (filament bought is stock), with filament wasted on failed prints as its own P&L line; a replayed Salla/Zid/carrier webhook refused for thirty days across restarts; and the Arabic review fixes. Supersedes **v3.10.0**, published 2026-09-26 ([releases/latest](https://github.com/khaytapp/Khayt/releases/latest)), a minor cut and tagged the same day, all three platforms: choosing which products go on the web store (an "On store" box per product, stored on the record so the Mac sees it) and a review of the listings before publishing, pricing a multi-plate 3MF on every plate's time and filament, and the storefront catalogue built in one shared module the native Mac app publishes through too. Supersedes **v3.9.2**, published 2026-09-25 ([releases/latest](https://github.com/khaytapp/Khayt/releases/latest)), a patch cut and tagged the same day: the first Electron release with a macOS build since v3.7.0 (BUILD_MAC is on until the native Mac app reaches beta, and build-mac now launches the notarized zip before publish), plus cloud sync that stops retrying a refusal it cannot fix (401/403/412/413) and shows the cloud's reason. Supersedes **v3.9.1**, published 2026-09-24 ([releases/latest](https://github.com/khaytapp/Khayt/releases/latest)), a patch cut from `main` and tagged the same day: the file-safety fixes (library deletes go to the Trash, a new ZATCA key keeps the old one, the Saudi-night daily backup no longer overwrites the previous day, a full wipe keeps one copy of the book), separate backup pools for dailies and sync snapshots, and security fixes (a phone could overwrite settings or printer keys; slicer settings are per-computer; the ntfy topic and webhook URLs stay off the cloud). `BUILD_MAC` is still false. Supersedes **v3.9.0**, published 2026-09-23 ([releases/latest](https://github.com/khaytapp/Khayt/releases/latest)), cut from `main` and tagged the same day with no beta line: two cloud-sync fixes (a shop near its plan's size, and a shop whose cloud copy was gone, could each stop syncing for good), two security fixes (carrier API keys and webhook secrets stored in the clear; outbound addresses into a Tailscale network), online Salla/Zid orders taking stock off the shelf, and the integrations directory following the shop's country rather than the interface language. `BUILD_MAC` is still false, so it carries no macOS build either. Supersedes **v3.8.0**, published 2026-09-21 ([releases/latest](https://github.com/khaytapp/Khayt/releases/latest)), cut from `main` with **no beta line at all** — `3.8.0` ran no pre-release, so nothing on it was soaked before the stable tag; the previous stable had `beta.25` behind it. **`BUILD_MAC` was deliberately left false for this cut**, so `v3.8.0` carries no macOS build and `carry-mac-manifest` republishes `v3.7.0`'s `latest-mac.yml` under it: a macOS shop on the Electron app stays on 3.7.0 and is not offered 3.8.0. That is the intended direction now that Khayt for macOS is its own app. Supersedes v3.7.0 (2026-09-11). The previous stable entry read: `3.7.x` — latest tag **v3.7.0**, published 2026-09-11 ([releases/latest](https://github.com/khaytapp/Khayt/releases/latest)), cut from `main` rather than promoted from `v3.7.0-beta.25` unchanged — 225 commits had landed since that cut. Supersedes v3.6.0 (2026-08-21), which superseded v3.5.3 (2026-08-01)
