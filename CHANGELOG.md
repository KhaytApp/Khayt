...
- **(Mac) A job going out the door can be labelled here.** The filament shelf
  ...
  anything is printed, and the same code on the label: a link to the customer's
  own page when the shop's cloud is connected, and a code the shop's own phone
  reads otherwise.
...
- **Khayt for macOS updates itself.** The native Mac app had no way to: a shop
  ...
  Updates…* item in the Khayt menu, and asks on first launch whether to look on
  its own rather than deciding for you. An update has to pass two separate
...
Two shelves and a library. The library takes twenty-two kinds of file instead
of six, opens RAR and 7-Zip packs, has stopped refusing anything over 32 MB,
and sends a deleted model to the Trash. The filament shelf can be added to —
it could not be, and nothing said so. And the other shelf, the one holding
glue and bags and nozzles, can be kept here at all for the first time.
...
- **(Mac) The other shelf can be kept here: glue, IPA, bags, nozzles.** Khayt
  ...
  shop that uses the Mac had an empty shelf and a card that could only ever
  say nothing, which reads as "nothing is running out". A consumable can be
  added, corrected and taken off here now, and a shelf with nothing on it yet
  says so and offers the way to fill it rather than hiding itself. And the
  card was only ever the urgent half: the rule it draws deliberately leaves
  out anything that is neither low nor due soon, so the rest of the shelf was
  invisible even to a shop that had one. It lists everything now, low first,
  with what the rule said about cover and quantity kept on the rows it spoke
  about. Shelves —
  Packaging, Spares, Cleaning, whatever the shop calls them — can be typed on
  an item and picked between, one spelling each however it is typed, with the
  things nobody filed reachable under their own heading rather than only
...
- **Your Profit & Loss will show a lower profit, and the old figure was wrong.**
  ...
  liability you hold until you remit it, never as income. The report was taking
  only expenses off the full amount charged, so it counted the VAT as profit —
  on the sample shop, 20,664.08 shown against 17,337.45 actually kept, 19% high
  in exactly the figure you would look at to decide whether the quarter made
...
- **You can now record the VAT you PAY, and it stops being a cost.** There is a
  ...
  exempt purchase carries none. Enter it and that tax comes off the tax you
  charged, so the quarterly report shows what you actually owe rather than what
  you collected, and it can be negative in a quarter you buy a printer. On a
  spool it also means a job is costed at what the plastic really cost you, so
  your margins compare like with like. Every expense and every spool already in
