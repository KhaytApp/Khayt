...
|---------|----------------|-------|
| **Stable** | **v3.10.1** (2026-09-26) — a patch cut and tagged the same day, **all three platforms**: net profit that counted filament twice now counts it once, when it is used (filament bought is stock), with filament wasted on failed prints as its own P&L line; a replayed Salla/Zid/carrier webhook refused for thirty days across restarts; and the Arabic review fixes. Previous stable: **v3.10.0** (2026-09-26) — a minor cut and tagged the same day, **all three platforms**: choosing which products go on the web store (an "On store" box per product, stored on the record so the Mac sees it) and a review of the listings before publishing, pricing a multi-plate 3MF on every plate's time and filament, and the storefront catalogue built in one shared module the native Mac app publishes through too. Previous stable: **v3.9.2** (2026-09-25) — a patch cut and tagged the same day, **all three platforms**: the first Electron release with a macOS build since v3.7.0 (BUILD_MAC is on until the native Mac app reaches beta, and build-mac now launches the notarized zip before publish), plus cloud sync that stops retrying a refusal it cannot fix (401/403/412/413) and shows the cloud's reason. Previous stable: **v3.9.1** (2026-09-24) — a patch cut from `main` and tagged the same day, **no macOS build** (`BUILD_MAC` still false). It carries the file-safety fixes (library deletes go to the Trash, a new ZATCA key keeps the old one, the Saudi-night daily backup no longer overwrites the previous day, a full wipe keeps one copy of the book), separate backup pools for dailies and sync snapshots, and security fixes (a phone could overwrite settings or printer keys; slicer settings are per-computer; the ntfy topic and webhook URLs stay off the cloud). Previous stable: **v3.9.0** (2026-09-23) — cut from `main` and tagged the same day, **no beta line** and **no macOS build** (`BUILD_MAC` still false). It carries two cloud-sync fixes (a shop near its plan's size, and a shop whose cloud copy was gone, could each stop syncing for good), two security fixes (carrier API keys and webhook secrets stored in the clear; outbound addresses into a Tailscale network), online Salla/Zid orders taking stock off the shelf, and the integrations directory following the shop's country rather than the interface language. Previous stable: **v3.8.0** (2026-09-21) — cut from `main` with **no beta line**, and **no macOS build** (`BUILD_MAC` left false by the maintainer, so `carry-mac-manifest` republishes `v3.7.0`'s `latest-mac.yml` and a macOS shop on the Electron app is not offered 3.8.0). It carries the report corrections — sixty-nine figures across Reports, the P&L, expenses, waste and machine cost had been computed from part of the book, so **figures a shop has looked at before will move** — the redesigned window as the default, quoting straight from the catalogue, a content policy on the customer-facing Khayt Online pages that names every route an injection could leave by, and 178 fields whose label a screen reader can now follow. The 3.8.0 gate and the condition NOT met are recorded below. Previous stable: **v3.7.0** (2026-09-11) | [Latest release](https://github.com/khaytapp/Khayt/releases/latest) — **the 3.7.0 line, cut from `main` rather than promoted from `v3.7.0-beta.25` unchanged**, because 225 commits had landed since that cut, including the money corrections and three dependency security fixes. Khayt stops guessing what a print cost and reads it: a completed job is offered the time and filament the PRINTER measured rather than the estimate under a second name, that figure is what comes off the shelf, and cost-against-quote is reported per model and per machine. **It moves money figures** — revenue no longer counts VAT you collect, and VAT you PAY on stock is recordable — so it carries a `### Before you update` section and the update gates on it. It also brings the Electron floor current: three unapplied Chromium/V8/ANGLE/WebRTC patch releases and two dependency advisories, one held open by this repo's own version pin. The 3.7.0 promotion gate and the one condition WAIVED are recorded below. |
...
## The 3.8.0 promotion gate — how it was settled (2026-09-21)

`v3.8.0` was tagged stable on 2026-09-21 from `main`, three days after the
section was written — the notes were reconciled with what the build had grown to
in the meantime. Recorded here rather than
left as a step that quietly did not happen — and one condition was **not met**,
by decision rather than by oversight.

- **Seven consecutive days with no new cut: MET ON THE LINE, AND THERE IS NO
  ARTIFACT.** `v3.7.0` was published 2026-09-11 and nothing was cut on the
  Electron line for seven days. But `3.8.0` ran **no pre-release at all**: where
  `3.7.0` could at least point at `beta.25` as the build people had been
  running, this line has no soaked predecessor. Every commit in it reaches a
  shop for the first time as a stable tag. That is a weaker position than
  `3.7.0`'s met-on-the-line, and it is stated rather than ticked.
- **macOS current at promotion: NOT MET, DELIBERATELY.** `BUILD_MAC` is `false`
  and was left that way on the day. `v3.8.0` therefore carries no macOS build,
  `carry-mac-manifest` republishes `v3.7.0`'s `latest-mac.yml` beneath it, and a
  macOS shop running the Electron app stays on 3.7.0 rather than being offered
  this release. The condition was written when the Electron app was the only
  app; **Khayt for macOS is now its own product on its own version line**, and
  pushing Electron builds at macOS is no longer the direction. That is the
  reason, and it is a decision about product rather than a skipped step. **The
  condition is not retired**: it applies again the moment a macOS shop is
  expected to take an Electron build.
- **Eviction exercised on a real library and brought back: STILL WAIVED, on the
  same grounds as 3.7.0.** Nothing in this release touches tiering, so the basis
  of that waiver — eviction is off until a shop configures a provider, and none
  has — is unchanged. It still transfers to the first release that ships tiering
  ON, or the first shop that configures a provider.
