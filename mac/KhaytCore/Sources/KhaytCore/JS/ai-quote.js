...
function draftToPart(draft, { inventory = [], defaults = {}, reclaimsTax = false } = {}) {
  ...
  const part = {
    ...
    supportWeight: 0,
    // HOURS, because that is what `printTime` means on every other part in
    // this codebase — the calculator, the store, the invoice and both apps'
    // editors all read it as hours.
    //
    // This returned MINUTES, straight from the model's `printTimeMin`, in a
    // field named for something else. It was not wrong on screen only because
    // the one call site remembered to divide: `renderer/build.js` wrote
    // `(part.printTime / 60).toFixed(2)` into the form. A second caller that
    // did not know — and the Mac was about to be one — would have quoted a
    // ninety-minute print as ninety hours.
    printTime: Math.max(0, +draft.printTimeMin || 0) / 60,
