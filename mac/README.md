...
and the order tracking page are here; the calendar feed, the webhooks, the kiosk
...
connect, and a different status for "you have no session" — 401 and 403. Only
one answers for a given machine, so the surface that did is remembered and
tried first; trying the wrong one costs a failed request every few seconds
