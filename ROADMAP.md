...
## Now (post-3.10.1, the Electron line closed — on `main`)

**Stable is v3.10.1**, published 2026-09-26 — a patch cut and tagged the same day, **all three platforms**: net profit that counted filament twice now counts it once, when it is used (filament bought is stock), with filament wasted on failed prints as its own P&L line; a replayed Salla/Zid/carrier webhook refused for thirty days across restarts; and the Arabic review fixes. **Net profit figures move up.** The previous stable was v3.10.0, published 2026-09-26 — a minor cut and tagged the same day, **all three platforms** (BUILD_MAC stays on until the native Mac app reaches beta): choosing which products go on the web store (an "On store" box per product, stored on the record so the Mac sees it) and a review of the listings before publishing, pricing a multi-plate 3MF on every plate's time and filament, and the storefront catalogue built in one shared module the native Mac app publishes through too. The previous stable was v3.9.2, published 2026-09-25 — a patch cut and tagged the same day: the first Electron release with a macOS build since v3.7.0 (BUILD_MAC is on until the native Mac app reaches beta, and build-mac now launches the notarized zip before publish), plus cloud sync that stops retrying a refusal it cannot fix (401/403/412/413) and shows the cloud's reason. **All three platforms**, so a macOS shop on the Electron app moves from 3.7.0 to 3.9.2. The previous stable was v3.9.1, published 2026-09-24 — a patch cut from `main` and tagged the same day, carrying the file-safety fixes (library deletes go to the Trash, a new ZATCA key keeps the old one, the Saudi-night daily backup no longer overwrites the previous day, a full wipe keeps one copy of the book), separate backup pools for dailies and sync snapshots, and security fixes (a phone could overwrite settings or printer keys; slicer settings are per-computer; the ntfy topic and webhook URLs stay off the cloud). `BUILD_MAC` still false. The first release built with both the Linux and the Windows launch gates. The previous stable was v3.9.0, published 2026-09-23 — cut from `main` the day it was tagged, with no beta line, carrying two cloud-sync fixes (a shop near its plan's size, and a shop whose cloud copy was gone, could each stop syncing for good), two security fixes (carrier API keys and webhook secrets stored in the clear; outbound addresses into a Tailscale network), online Salla/Zid orders taking stock off the shelf, and the integrations directory following the shop's country rather than the interface language. **No macOS build**: `BUILD_MAC` is still false, so a macOS shop on the Electron app stays on 3.7.0. It is the first release whose Linux build was launched by `build-linux` before it went public. The previous stable was v3.8.0, published 2026-09-21 — cut from `main` with no beta line, carrying the report corrections (sixty-nine figures that had been computed from part of the book), the redesigned window as the default, quoting from the catalogue, the Khayt Online content-policy hardening and the label fix that lets a screen reader name a field. **No macOS build**: `BUILD_MAC` was left false, so a macOS shop on the Electron app stays on 3.7.0. The previous stable was v3.7.0, PUBLISHED 2026-09-11 on all three platforms and verified
